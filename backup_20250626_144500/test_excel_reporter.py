    ...
    try:
        ...
        with pd.ExcelWriter(final_report_path, engine='xlsxwriter') as writer:
            ...
            if 'Handling Fee' in transaction_df.columns:
                # 월별 Handling Fee
                handling_by_month = transaction_df.groupby('월')['Handling Fee'].sum().reset_index()
                handling_by_month.columns = ['월', '월별HandlingFee합계']
                handling_by_month.to_excel(writer, sheet_name='04_월별HandlingFee', index=False)
                
                # 창고별 Handling Fee
                handling_by_location = transaction_df.groupby('Location')['Handling Fee'].sum().reset_index()
                handling_by_location.columns = ['창고/현장', '총HandlingFee합계']
                handling_by_location = handling_by_location.sort_values('총HandlingFee합계', ascending=False)
                handling_by_location.to_excel(writer, sheet_name='05_창고별HandlingFee', index=False)
                
                # Handling Fee 통계
                handling_stats_df = pd.DataFrame({
                ...
                handling_stats_df.to_excel(writer, sheet_name='06_HandlingFee통계', index=False)
